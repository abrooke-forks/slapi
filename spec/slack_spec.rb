...
RSpec.describe 'integration test', skip: (!ENV['SLACK_API_TOKEN'] || !ENV['CONCURRENCY']) && 'missing SLACK_API_TOKEN and/or CONCURRENCY' do
  ...
  before do
    ...
    Slack.configure do |slack|
      slack.logger = logger
      slack.token = settings.SLACK_API_TOKEN
    end
  end
  ...
  let(:connection) do
    ...
    client.start_async do |driver|
      driver.on :open do |data|
        logger.info "connection.on :open, data=#{data}"
        queue.push nil
      end
    end
  end
  ...
  def wait_for_server
    ...
    queue.pop_with_timeout(5)
    logger.info '#wait_for_server, joined'
  end

  def stop_server
    logger.info '#stop_server'
    client.stop!
    logger.info '#stop_server, stopped'
  end
  ...
  context 'client connected' do
    ...
    it 'responds to message' do
      ...
      logger.info "chat_postMessage, channel=#{channel}, message=#{message}"
  ...
  it 'gets hello' do
    client.on :hello do |data|
      logger.info "client.on :hello, data=#{data}"
      expect(client.started?).to be true
      client.stop!
    end
  ...
  it 'gets close, followed by closed' do
    ...
    client.on :close do |data|
      logger.info "client.on :close, data=#{data}"
      expect(client.started?).to be true
      @close_called = true
    end

    client.on :closed do |data|
      logger.info "client.on :closed, data=#{data}"
